...
- `SearchInput.tsx` — Controlled search box with live query input. Displays "No closely matching results found" when a query has no results above the relevance threshold. Wired to `useSearchRanking` hook. Force-directed mode only (swim-lane search support documented as deferred in design-notes.md §27).
