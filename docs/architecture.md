...
- `OptionsPanel.tsx` — Persistent icon-only hamburger button (three horizontal lines) in the top-right corner with accessible name "Options & help" via `aria-label`, opening a panel that hosts the `LayoutModeToggle`, `ThemeToggle`, and a force-directed-only "Reset view" button, plus explanatory text for each mode. Dismissible via overlay click or close button. Does not navigate.
...
- `GraphCanvas.tsx` — Wrapper around `react-force-graph-2d`; renders nodes (colored by folder taxonomy via validated categorical palette) with small status dots (active/revisiting/dormant), edges, and hover tooltips showing node title and status. Click-to-center-zoom interaction animates over ~900ms. Initial layout uses `fitView()` with a "chase-fit" mechanism (re-fitting on a 300ms interval for 9 seconds) to track the force-directed simulation's gradual node-position settling rather than a single snapshot (Epic niaTair). When a node is focused (via direct click or external `focusedNodeId` prop), a selection ring draws around the focused node, unrelated nodes/edges dim to 15% opacity, and connected edges/nodes render in emphasized blue. Custom canvas draw callbacks (`nodeCanvasObject`, `nodePointerAreaPaint`) handle visual rendering, with support for dark-mode-aware text/label colors (passed via `isDark` prop). Node labels are hidden below a zoom level of 2 and restored above it to maintain legibility at wide-fit zoom (Epic niaTair). Theme-aware edge colors differentiate normal (25% opacity), dimmed (6% opacity), and emphasized (blue) link states in both dark and light themes. Accepts optional `onNodeClick` callback and `searchScores`/`relevanceThreshold` props to dim non-matching nodes via `ctx.globalAlpha` during live search. Also accepts `filteredOutNodeIds` (set of node IDs to dim due to status/folder filters) and `radiusScaleByNodeId` (Map of node ID to radius scale factor for the edge-count indicator feature). Additionally accepts `focusedNodeId` (string or null) — when an external source (e.g., a chip click in SidePanel) sets this prop, the effect triggers `centerAt`/`zoom` on the target node, applying the same focus treatment a direct canvas click gets (design-notes.md §31). New `onResetViewReady` callback prop hands the fit function to the parent (`app/graph/page.tsx`), allowing external controls (the "Reset view" button in `OptionsPanel`) to trigger the same focus-frame behavior (Epic niaTair).
