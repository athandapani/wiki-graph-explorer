...
|---|---|---|
| `/` | `app/page.tsx` | Home/landing page with product intro, "How to use it" instructions (swim-lane, force-directed, theme/layout toggles, semantic search), and CTA link to `/graph`. Desktop layout (`lg:` breakpoint and above) uses `lg:grid-cols-2` with text content in left column and a decorative, non-interactive `HomeHeroGraphic` SVG component (abstract node network with staggered opacity-pulse animation) in right column. Below `lg:` breakpoint, layout reverts to single column with hero graphic hidden. The `HomeHeroGraphic` component is marked `aria-hidden="true"`, uses `currentColor` for theming, and includes abstract node/edge data with a `hero-pulse` keyframe animation (defined in `app/globals.css`) that applies staggered delays to each node circle. Originally rewritten in Epic scQi8pt away from boilerplate; hero layout added in Epic hxffZxb. |
...
- `SwimLaneCanvas.tsx` — Custom SVG/CSS-based renderer (not `react-force-graph-2d`). Displays nodes as labeled pill buttons organized into horizontal lanes (top 4 folders by count + "Other" overflow lane). Each lane renders as a tinted rounded container (`backgroundColor: {folderColor}1a` dark, `0f` light) with a folder heading (`<h3>` uppercase), a page-count descriptor (`"N pages total"`), and optional pills inside a flex-wrap layout. Minimum lane height is `MIN_LANE_HEIGHT_PX = 84` to ensure headings/descriptors are never clipped; remaining space is distributed proportionally by visible node count. Click-to-reveal ~950ms curved connector-line animations draw from clicked node to its edges, with folder-colored strokes, rendered via SVG at z-index -10 (behind pill text). Nodes with zero edges are permanently hidden; nodes with one or fewer edges are hidden by default and revealed with dashed borders (plus full opacity) when a connected node is clicked, or when a search match reveals them. A "+N more" affordance button appears in each lane when hidden nodes exist and the lane is not yet expanded. Supports dark-mode-aware colors. No pan/zoom (fixed layout). Accepts `focusedNodeId` (string or null) — when an external source (e.g., a chip click in SidePanel) sets this prop during render, the component syncs its `activeNodeId` state via React's render-phase pattern, triggering the same connector-line and highlight treatment a direct pill click gets (design-notes.md §31). The focus sync is one-way and non-null-only; dismissing the panel (setting focusedNodeId to null) does not clear the board's current highlight. Memos: `zeroDegreeIds` tracks nodes with no connections (never revealed), `hiddenCandidateNodes` collects zero-degree and low-degree nodes not already on the board via click or search reveal, `expandedLaneNames` state tracks which lanes have had their "+N more" expanded.
